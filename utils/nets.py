...
def prepare_optimizer(net, lr, momentum, adam, nesterov: bool = False, rmsprop_alpha: float = None):
    if rmsprop_alpha is not None:
        return T.optim.RMSprop(net.parameters(), lr=lr, alpha=rmsprop_alpha)
    if adam:
